...
model <- train(Exitus_RITS ~ SOFA + glu_fac, data = analize_nona_model, method = "glm", 
               trControl = train_control, 
               family = "binomial")
...
## Modelis tik su Atvykimo gliukoze

```{r echo= FALSE, message=FALSE, warning=FALSE}
analize_nocd <- analize %>%
  select(Atvykimo_glu, Exitus_RITS, CD) %>%
  filter(CD == 0) %>%
  na.omit()

log.glu <- glm(Exitus_RITS ~ Atvykimo_glu, data=analize_nocd, family='binomial')
...
plot_log <- ggplot(analize_nocd, aes(x=Atvykimo_glu, y=Exitus_RITS)) + geom_point() +
  ...
  ylab('Exitus')+
  xlab("Glikemija, mmol/l")
plot_log

ggsave('logistine_kreive.png', width = 5, height = 5, bg='white')
```
